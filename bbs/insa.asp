...
'************************************************************************************
	...
	If cd2 <> "" Then					'검색조건이 있는 경우
		param = " WHERE "& cd1 &" LIKE '%"& cd2 &"%' AND bbs_id = "& bbs_id &" AND cid = '"& USER_COMP &"' "
	Else								'검색조건이 없는 경우
		param = " WHERE bbs_id = "& bbs_id &" AND cid = '"& USER_COMP &"' "
	End If
...
$(document).ready(function(){
	...
	$("#btnReset").click(function (){		//새로고침
		document.location = "insa.asp";
	});
	$("#btnWrite").click(function (){		//글쓰기
		document.location = "insa_w.asp";
	});
});
function goSearch(){
	$("#fm1").attr({action:"insa.asp", method:"post"}).submit();
}
...
%>
										<tr height=40>
											...
											<td class="pl10">
												<a href="insa_v.asp?seq=<%=rs("seq")%>&page=<%=page%>&<%=pageParam%>">
