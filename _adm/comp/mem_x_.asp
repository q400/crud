...
'	Response.Write "hopeID : "& hopeID &"<br>"
...
'		Response.Write "iwidth : "& iwidth &"<br>"
		...
		If LCase(ext)="asp" Or LCase(ext)="asa" Or LCase(ext)="aspx" Or LCase(ext)="xml" Then
			...
			Call AlertGo("등록 불가능 확장자입니다.","modify_a.asp?uid="& uid &"&"& pageParam)
...
'			If iwidth >= 150 And iwidth <= 200 Then			'150~200 size = 정상
				If True = objImage.SetSourceFile(upFile.TempFilePath) Then		'임시저장
					...
					'썸네일 20 size
					Call objImage.SaveasThumbnail(serverPath &"\"& fileNm1, objImage.ImageWidth/iwidth*20, objImage.ImageHeight/iheight*ratio20, False)
					'썸네일 50 size
					Call objImage.SaveasThumbnail(serverPath &"\"& fileNm2, objImage.ImageWidth/iwidth*50, objImage.ImageHeight/iheight*ratio50, False)
					'정상사진
					Call objImage.SaveasThumbnail(serverPath &"\"& fileNm3, objImage.ImageWidth/iwidth*150, objImage.ImageHeight/iheight*ratio150, False)
				End If
'			Else
...
<input type="hidden" name="cd5" value="<%=cd5%>">
<input type="hidden" name="cd6" value="<%=cd6%>">
