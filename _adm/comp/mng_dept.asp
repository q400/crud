...
<!-- <input type="hidden" name="page" id="page" value="<%=page%>" /> -->
						...
						<table width=100% border="0" cellspacing="0" cellpadding="0" class="pr50">
							<tr>
								<td class="pb20"><a href="/_adm/">관리자</a> > 고객관리 > 고객사 부서 관리</td>
							</tr>
							<tr>
								<td>
									<div class="mb10">
...
<%
		rso()
		SQL = " SELECT TOP 20 * FROM codt010 "& param1 _
			& " AND seq NOT IN (SELECT TOP "& ((page-1) * 20) &" seq FROM codt010 "& param1 _
			& " ORDER BY code_se, code_id) " _
			& " ORDER BY code_se, code_id "
'		Response.write SQL
...
%>
											<tr height=30>
												<td class="ct">
													<a href="javascript:;" onclick="unoPop('code_v.asp?seq=<%=rs("seq")%>','코드수정',420,350)"><%=rs("code_se")%></a>
												</td>
												<td class="ct">
													<a href="javascript:;" onclick="unoPop('code_v.asp?seq=<%=rs("seq")%>','코드수정',420,350)"><%=rs("code_id")%></a>
												</td>
												<td class="ct">
													<a href="javascript:;" onclick="unoPop('code_v.asp?seq=<%=rs("seq")%>','코드수정',420,350)"><%=rs("use_yn")%></a>
												</td>
												<td class="pl20">
													<a href="javascript:;" onclick="unoPop('code_v.asp?seq=<%=rs("seq")%>','코드수정',420,350)"><%=rs("code_nm")%></a>
												</td>
<%
				If rs("code_se") = "팀" Then
%>
												<td class="ct">
													<a href="javascript:;" onclick="unoPop('code_v.asp?seq=<%=rs("seq")%>','코드수정',420,350)"><%=cmnCode("시간",rs("opt1"))%></a>
												</td>
												<td class="ct">
													<a href="javascript:;" onclick="unoPop('code_v.asp?seq=<%=rs("seq")%>','코드수정',420,350)"><%=cmnCode("시간",rs("opt2"))%></a>
												</td>
												<td class="ct">
													<a href="javascript:;" onclick="unoPop('code_v.asp?seq=<%=rs("seq")%>','코드수정',420,350)"><%=cmnCode("시간",rs("opt3"))%></a>
												</td>
												<td class="ct">
													<a href="javascript:;" onclick="unoPop('code_v.asp?seq=<%=rs("seq")%>','코드수정',420,350)"><%=cmnCode("시간",rs("opt4"))%></a>
												</td>
<%
				End If
%>
...
%>
						...
						<div class="ct mt10 pb50">
							...
							//-->
							<button type="button" onclick="unoPop('code_w.asp','코드등록',420,250)" class="btn01">신규등록</button>
						</div>
</form>
					</td>
				</tr>
