...
'************************************************************************************
	...
	rso()
	SQL = " SELECT RIGHT(CONCAT('00', (ISNULL(MAX(RIGHT(cid, 3)) + 3, '2'))), 3) as mxNum " _
		& " FROM cmpt010 " _
		& " WHERE cid NOT IN ('00000','ZZ999') " _
		& " AND YEAR(ddate) = YEAR(getdate()) "
	rs.open SQL, dbcon
	If Not rs.eof Then
		mxNum					= rs("mxNum")
	End If
...
$(document).ready(function(){
	$("#makeCd").click(function(){				//코드생성
		...
		let today = mon + day + year.substr(1, 3); //순서,자리수 유의

		for (let i = 0; i < 1; i++) {
			//rdm = Math.random();
			//str += chars.charAt(Math.floor(rdm * chars.length));
//			console.log('_________ Math.random() = '+ rdm);
//			console.log('_________ chars.length = '+ chars.length);
//			console.log('_________ 곱 = '+ rdm * chars.length);
//			console.log('_________ Math.floor = '+ Math.floor(rdm * chars.length));
//			console.log('_________ '+ str);
		}
//		return str;
		console.log('_________ '+ $('#induty_cl').val() + today + '<%=mxNum%>');

		$('#cid').val($('#induty_cl').val() + today + '<%=mxNum%>');		//고객코드 set
		$('input:radio[id="sttus3"]').attr('checked', 'checked');			//상태 [완료] 처리
	});
	...
	$("#smsSend").click(function(){				//SMS발송
		...
		console.log('_________ tel = '+ tel);
		$("#smsform > input[id='tel']").val(tel);
	});
...
function goSave(){
	...
	$("#fm1").attr({action:"rqst_x.asp", method:"post", target:"nullframe"}).submit();
}
...
<input type="hidden" name="flag" id=flag value="<%=flag%>" />
						...
						<table width=100% border="0" cellspacing="0" cellpadding="0" class="">
							<tr>
								<td class="pb20"><a href="/_adm/">관리자</a> > 서비스관리 > 사용신청수정</td>
							</tr>
							<tr>
								<td>
									<div class="mt10 mb10">
								...
								<td>
									<table width=100% border="0" cellspacing="0" cellpadding="0">
										...
										<tr>
											...
											<td height=40>
												<input type="text" name="cid" id="cid" maxlength="11" value="<%=cid%>" style="width:200px;" <%If cid <> "" Then%>readonly="readonly"<%End If%> />
...
%>
										...
										<tr>
											...
											<td height=40>
												<input type="text" name="opt1" id="opt1" maxlength="4" placeholder="0900" style="width:100px;" value="<%=opt1%>" />
												숫자 4자리 24시 표현 (예 0900)
											</td>
										</tr>
										<tr>
											<th><label>기본퇴근시간</label></th>
											<td height=40>
												<input type="text" name="opt2" id="opt2" maxlength="4" placeholder="1800" style="width:100px;" value="<%=opt2%>" />
												숫자 4자리 24시 표현 (예 1800)
											</td>
										</tr>
										<tr>
...
%>
							...
							<button type="button" onclick="location='rqst.asp?page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>'" class="">목록</button>
<%
		Else
%>
