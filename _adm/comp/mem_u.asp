...
'	Response.Write "<br>"& SQL &"<br>"
	...
	pageParam = "cid="& cid &"&cd1="& cd1 &"&cd2="& cd2 &"&cd3="& cd3 &"&cd4="& cd4 &"&cd5="& cd5 &"&cd6="& cd6 &"&cd7="& cd7
...
function goSave(){
	...
	*/
	$('#fm1').attr({action:'mem_x.asp', method:'POST',target:'nullframe'}).submit();
}
function pwdReset(){
	if(confirm('비밀번호를 12345로 리셋합니다.')){
		$('#flag').val('RESET');
		$('#fm1').attr({action:'mem_x.asp?uid=<%=uid%>&<%=pageParam%>&page=<%=page%>', method:'POST', target:''}).submit();
	}
}
function insaCard(uid){		//인사카드
	Popup('/adm/card/card.asp?uid='+ uid,900,1000,0,0,0,1,10);
}
function setAdmin(uid){		//관리자권한부여
...
<input type="hidden" name="cd1" id=cd1 value="<%=cd1%>" /><!-- 조회조건 -->
<input type="hidden" name="cd2" id=cd2 value="<%=cd2%>" /><!-- 조회조건값 -->
<input type="hidden" name="cd3" id=cd3 value="<%=cd3%>" /><!-- 팀 -->
<input type="hidden" name="cd4" id=cd4 value="<%=cd4%>" /><!-- 부서 -->
<input type="hidden" name="cd5" id=cd5 value="<%=cd5%>" /><!-- 퇴사자 -->
<input type="hidden" name="cd6" id=cd6 value="<%=cd6%>" /><!-- 직급 -->
<input type="hidden" name="cd7" id=cd7 value="<%=cd7%>" /><!-- 권한 -->
<input type="hidden" name="dept" id=dept value="<%=dept%>" />
<input type="hidden" name="team" id=team value="<%=team%>" />
<input type="hidden" name="ulvl" id=ulvl value="<%=ulvl%>" />
<input type="hidden" name="page" id=page value="<%=page%>" />
<input type="hidden" name="flag" id=flag value="<%=flag%>" />

						<table width=100% border="0" cellspacing="0" cellpadding="0" class="pr50">
							<tr>
								<td class="pb20"><a href="/_adm/">관리자</a> > 고객관리 > 회원상세정보</td>
							</tr>
							<tr>
								<td>
									<div class=""></div>
									<div class="ib mb5 frt">
										...
										<button type="button" onclick="setAdmin('<%=uid%>')" class="btn01">관리자권한부여</button>
									</div>
								</td>
							</tr>
							<tr>
...
%>
											...
											<tr height=30>
												...
												<td>
													...
													<select name="auth1" id="auth1" style="width:180px;">
														<option value="" selected>선택</option>
														<%=cmnCdList("권한",auth,"","","")%>
													</select>
												</td>
											...
											<tr height=30>
												...
												<td>
													<select name="hp1" id="hp1" style="width:80px;">
														<option value="010" <%If hp1="" Or hp1="010" Then%>selected<%End If%>>010</option>
...
<%
			rso()
			SQL = "	SELECT code_id, code_nm FROM codt010 WHERE code_se = '이메일' ORDER BY code_id ASC "
			rs.open SQL, dbCon, 3
			Do Until rs.eof
%>
														<option value="<%=rs("code_nm")%>" <%If rs("code_nm") = email2 Then%>selected<%End If%>><%=rs("code_nm")%></option>
<%
				rs.MoveNext
			Loop
			rsc()
...
function jusoCallBack(zipNo,roadFullAddr){
	...
	console.log('________ '+ roadFullAddr);
	console.log('________ '+ roadFullAddr.indexOf('�'));
	console.log('________ '+ roadFullAddr.indexOf('?'));
	if(roadFullAddr.indexOf('�') == -1 || roadFullAddr.indexOf('?') == -1){
		$("#zip").val(zipNo);
...
function goPopup(){
    ...
    //var pop = window.open("/jusoPopup.asp","pop","scrollbars=yes, resizable=yes");
											...
											<tr height=30>
												...
												<td>
													<div class="mt5">
														<input type="text" name="zip" id=zip value="<%=zip%>" style="width:80px;" maxlength=6 placeholder="우편번호" />
<!-- 														<button type="button" onclick="goPopup()">우편번호검색</button> --
											...
											<tr height=30>
												<th>권한설정</th>
												<td>
													<select name="auth" id="auth" style="width:180px;"><!-- 권한 -->
														<%=cmnCdList("권한",auth,"","",cid)%>
													</select>
												</td>
											</tr>
										</tbody>
