...
'************************************************************************************
	...
	upObj.DefaultPath = Server.MapPath(PATH_WORK)				'저장경로
	...
	cl							= chkWord(upObj("cl"))			'분류
	path						= upObj("path")					'헤당경로
	sttus						= SQLI(upObj("sttus"))			'작업진행상태
	...
	If flag = "W" Then							'쓰기
		...
		SQL = "	INSERT INTO bbst050	(bbs_id, cid, title, cl, uid, path, sttus, rgid, contents) " _
			& "	VALUES ( " _
			& "			"& bbs_id &", " _
			& "			'"& USER_COMP &"', " _
			& "			'"& title &"', " _
	...
	ElseIf flag = "R" Then						'댓글
		dbcon.Execute "BEGIN Transaction"

		SQL = " INSERT INTO	bbst013 (bbs_id, seq, cid, rgid, note) " _
			& "	VALUES	( " _
			& "			"& bbs_id &", " _
			& "			"& seq &", " _
			& "			'"& USER_COMP &"', " _
	...
	ElseIf flag = "RD" Then						'댓글삭제
		dbcon.Execute "BEGIN Transaction"
		SQL = " DELETE FROM bbst013 WHERE idx = "& idx
		dbcon.Execute SQL,,128
		dbcon.Execute "COMMIT Transaction"
	...
	ElseIf flag = "D" Then						'삭제
		...
		SQL = "	DELETE FROM bbst050 WHERE bbs_id = "& bbs_id &" AND seq = "& seq
		dbcon.Execute SQL
...
<%	ElseIf flag = "R" Or flag = "RD" Then %>
<script>
	//alert("처리되었습니다.");
	$("#fm1").attr({action:"<%=vpath%>_v.asp", method:"post", target:"_top"}).submit();
</script>
<%	ElseIf flag = "D" Then %>
<script>
	alert("삭제되었습니다.");
