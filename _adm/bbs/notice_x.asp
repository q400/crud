...
'************************************************************************************
	...
	upObj.DefaultPath = Server.MapPath(PATH_NOTICE)				'저장경로
	upObj.MaxFileLen = 20*1024*1024								'하나 20 MB로 제한

	bbs_id						= SQLI(upObj("bbs_id"))
	seq							= SQLI(upObj("seq"))
	...
	If flag = "W" Then							'쓰기
		...
		SQL = "	INSERT INTO bbst010	(bbs_id, title, cid, uid, noti_yn, atm_yn, contents) " _
			& "	VALUES ( " _
			& "			"& bbs_id &", " _
			& "			'"& title &"', " _
			& "			'"& USER_COMP &"', " _
