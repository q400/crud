...
function goSearch(){
	...
	$("#mm").val("<%=mm%>");
	$("#dd").val("<%=dd%>");
	$("#fm1").attr({action:"flexList.asp", method:"post"}).submit();
}
function prevMonth(){
	loadingShow();		//loading
	$("#yy").val("<%=Year(vDate-1)%>");
						...
						<table width=100% border="0" cellspacing="0" cellpadding="0" class="pr50">
							...
							<tr>
								<td>
									<div class="mt10 mb10">
										<span>
											<button type="button" onclick="goUrl('flex.asp?yy=<%=yy%>&mm=<%=mm%>&dd=01')">달력보기</button>
								...
								<td>
									<table width=100% border="0" cellspacing="0" cellpadding="0" class="tbl900">
										<colgroup>
											...
											<col style="width:10%;" /><!-- 기본근무시간 -->
											<col style="width:10%;" /><!-- 적용근무시간 -->
...
<%
		rso()
		SQL = " SELECT TOP 15 A.*, B.unm, B.dept, B.team, C.* "_
			& "	FROM vact010 A " _
...
%>
											<tr height=34 align="center">
												...
												<td><!-- 기본근무시간 -->
													<a href="javascript:;" onclick="unoPop('flexPop.asp?vdt=<%=rs("vdate")%>&seq=<%=rs("seq")%>&idx=<%=rs("idx")%>','탄력근무설정',700,700);" title="<%=rs("seq")%>:<%=rs("idx")%>">
													<%=cmnCd2(USER_TEAM, "opt1")%>~<%=cmnCd2(USER_TEAM, "opt2")%></a>
												</td>
												<td><!-- 적용근무시간 -->
													<a href="javascript:;" onclick="unoPop('flexPop.asp?vdt=<%=rs("vdate")%>&seq=<%=rs("seq")%>&idx=<%=rs("idx")%>','탄력근무설정',700,700);" title="<%=rs("seq")%>:<%=rs("idx")%>">
													<%=rs("stime")%>~<%=rs("etime")%></a>
...
%>
						...
						<div class="ct mt10 pb50 pr50">
							...
							<button type="button" id=reload onclick="location='flexList.asp'">새로고침</button>
