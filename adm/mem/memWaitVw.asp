...
<%
'************************************************************************************
'* 화면명	: memWaitVw.asp - 대기(신입)직원관리(관리자)
'************************************************************************************
...
'	Response.Write "flag : "& flag &"<br>"
	...
	If uid = "" Then
		Call AlertGo("ID가 선택되지 않았습니다.","memWait.asp?page="& page &"&"& pageParam)
	End If
...
function goSave(){
	...
	*/
	$("#fm1").attr({action:"modifyWait_x.asp", method:'post'}).submit();
}
function pwdReset(){
	if(confirm("비밀번호를 12345로 리셋합니다.")){
		$("#flag").val("RESET");
		$("#fm1").attr({action:"modifyWait_x.asp?uid=<%=uid%>&<%=pageParam%>&page=<%=page%>", method:"POST", target:""}).submit();
	}
}
...
<input type="hidden" name="flag" id=flag value="<%=flag%>">
						...
						<table width=100% border="0" cellspacing="0" cellpadding="0" class="pr50">
							<tr>
								<td class="pb20">관리자 > 대기(신입)직원관리 > 직원상세정보</td>
							</tr>
							<tr>
								<td>
									<div class=""></div>
...
<%		End If %>
						...
						<div class="mt10 ct">
							...
							<button type="button" onclick="document.location='memWait.asp?<%=pageParam%>&page=<%=page%>'">목록</button>
						</div>
</form>
					</td>
				</tr>
