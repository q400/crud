...
'	If cd4 <> "" Then cd2 = ""				'부서에 값이 있으면 cd2를 ""처리
	...
	If cd1 = "hp" Then cd1 = "hp1 + hp2 + hp3" End If
...
<input type="hidden" name="page" value="<%=page%>">
						...
						<table width=100% border="0" cellspacing="0" cellpadding="0" class="pr50">
							<tr>
								<td class="pb20">관리자 > 직원관리</td>
							</tr>
							<tr>
								<td>
									<div class="ib mb5">
										<button type="button" onclick="location='memWait.asp'">대기자목록</button>
...
'		Response.Write SQL &"<br>"
		...
		If Not (rs.eof And rs.bof) Then
			...
			Select Case rs("auth")
				Case "AA0900" : auth = "일반"
				Case "AA0700" : auth = "<span class='blue fcb'>팀장</span>"
				Case "AA0500" : auth = "<span class='redyellow fcb'>부서장</span>"
				Case "AA0070" : auth = "<span class='yellow fcb'>부관리자</span>"
				Case "AA0050" : auth = "<span class='yellow fcb'>관리자</span>"
				Case "AA0001" : auth = "<span class='yellow fcb'>수퍼관리자</span>"
				Case Else : auth = "대기"
			End Select

				If rs("open_yn") = "N" Then
%>
										<tr align="center" bgcolor="#505050" height=34>
<%
				Else
%>
...
%>
											...
											<td>
												<a href="modify.asp?uid=<%=rs("uid")%>&<%=pageParam%>&page=<%=page%>"><b class="fcy"><%=rs("unm")%></b></a>
<%
				If USER_AUTH <= 50 Then
%>
												<img src="/img/icon/delete02.gif" class="vm pt" onclick="goDelete('<%=rs("uid")%>');" title="퇴사처리" />
<%
				End If
%>
											</td>
											<td>
												<a href="modify.asp?uid=<%=rs("uid")%>&<%=pageParam%>&page=<%=page%>"><%=rs("uid")%></a>
											</td>
											<td>
												<a href="modify.asp?uid=<%=rs("uid")%>&<%=pageParam%>&page=<%=page%>"><%=rs("hp1") &"-"& rs("hp2") &"-"& rs("hp3")%></a>
											</td>
											<td>
												<a href="modify.asp?uid=<%=rs("uid")%>&<%=pageParam%>&page=<%=page%>"><%=cmnCd1("부서",rs("dept"),"")%></a>
											</td>
											<td>
												<a href="modify.asp?uid=<%=rs("uid")%>&<%=pageParam%>&page=<%=page%>"><%=cmnCd1("팀",rs("team"),"")%></a>
											</td>
											<td>
												<a href="javascript:;" onclick="unoPop('/adm/latePop.asp?uid=<%=rs("uid")%>','지각관리',580,460);"
												class="hint--info hint--top" aria-label="지각관리"><%=cmnCd1("직급",rs("ulvl"),"")%></a>
											</td>
											<td>
<%				If USER_AUTH <= 50 Then %>
												<a href="javascript:makeAdmin('<%=rs("uid")%>');" class="hint--info hint--top" aria-label="관리자지정" /><%=auth%></a>
<%				Else %>
												<%=auth%>
<%				End If %>
											</td>
											<td>
												<a href="javascript:goActive('<%=rs("uid")%>');" class="hint--info hint--top" aria-label="활성화등록" /><%=rs("ddate")%></a>
											</td>
										</tr>
<%
							rs.MoveNext
...
%>
										<tr>
											<td height="1" colspan=10 background="/img/dot.png"></td>
										</tr>
<%
							End If
					i = i + 1
...
%>
						...
						<div class="ct mt10 pb50">
							...
							<button type="button" id=btnReset onclick="location='mem.asp'">새로고침</button>
