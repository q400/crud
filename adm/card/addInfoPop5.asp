...
<%
'************************************************************************************
'* 화면명	: addInfoPop5.asp - 직원부가정보 (급여정보) 관리
'************************************************************************************
...
$(document).ready(function(){
	...
	$("#btnAdd").click(function (){		//row 추가
		...
		list = list + "<tr height=34>"
				+ "<td><span style=\"display:none;\"><em>*협상일자</em></span></td>"
				+ "<td><input type=\"text\" name=\"pay_date\" id=pay_date style=\"width:90%;\" maxlength=10 placeholder=\"예)2001.03.09\" /></td>"	//협상일자
				+ "<td><span style=\"display:none;\"><em>*급여액</em></span><input type=\"text\" name=\"pay_note\" id=pay_note style=\"width:95%;\" maxlength=200 /></td>"	//내용
				+ "<td><input type=\"text\" name=\"pay_amt\" id=pay_amt style=\"width:90%;\" maxlength=20 placeholder=\"숫자만\" /></td>"		//급여액
				+ "<td><input type=\"text\" name=\"pay_rto\" id=pay_rto style=\"width:90%;\" maxlength=5 placeholder=\"숫자만\" /></td>"		//변동률
				+ "<td><input type=\"text\" name=\"pay_rsn\" id=pay_rsn style=\"width:90%;\" maxlength=200 /></td>"	//사유
				+ "</tr>";
		$("tbody").append(list);
		$("#add").remove();
	});
	$("#btnDelete").click(function (){	//선택삭제
		if($("input[name=chk]:checked").length == 0){
			...
			if(confirm("선택 항목을 삭제합니까?")){
				$("#flag").val("D");
				$("#fm1").attr({action:"addInfoPop5_x.asp", method:"POST", target:"nullframe"}).submit();
			}
		}
	});
	$("#btnReload").click(function (){
		$("#fm1").attr({action:"addInfoPop5.asp", method:"POST"}).submit();
	});
...
<div>
	<div class="mt5 ml5 mb5">
		...
		<button type="button" onclick="location='addInfoPop5.asp?uid=<%=uid%>'" class="btn03">급여정보</button>
			...
			<thead>
				<tr>
					<th height=30 colspan=10>
						<span class="flt mt5 ml20 f17 fc3">급여변동정보</span>
...
%>
				<tr height=34>
					...
					<td><input type="text" name="pay_date" id=pay_date style="width:100%;" maxlength=10 value="<%=rs("pay_date")%>" /></td><!-- 협상일자 -->
					<td><input type="text" name="pay_note" id=pay_note style="width:100%;" maxlength=200 value="<%=rs("pay_note")%>" /></td><!-- 내용 -->
					<td><input type="text" name="pay_amt" id=pay_amt style="width:100%;" maxlength=20 value="<%=rs("pay_amt")%>" /></td><!-- 급여액 -->
					<td><input type="text" name="pay_rto" id=pay_rto style="width:100%;" maxlength=5 value="<%=rs("pay_rto")%>" /></td><!-- 변동률 -->
					<td><input type="text" name="pay_rsn" id=pay_rsn style="width:100%;" maxlength=200 value="<%=rs("pay_rsn")%>" /></td><!-- 사유 -->
				</tr>
<%
			rs.MoveNext
			i = i + 1
