...
'		Response.Write "<br>SQL : "& SQL &"<br>"
	...
	Else		'신규입력
		uid = USER_ID
		sign3 = ""
		uDept = cmnCd1("부서",USER_DEPT,"")
		nmList = ""										'해당자 목록
	End If

	stime0 = cmnCd2(USER_TEAM, "opt1")					'소속 팀의 기준출근시간 CODE 가져오기
	etime0 = cmnCd2(USER_TEAM, "opt2")					'소속 팀의 기준퇴근시간 CODE 가져오기
...
//		console.log("inx ________"+ inx);
	...
	$("#btnSanc").click(function (){			//재가
		if(confirm("일괄 재가합니까?")){
			$("#opt").val("V");
			$("#fm1").attr({action:"flex_x.asp", method:"post"}).submit();
		}
	});
	$("#btnBundleSanc").click(function (){		//일괄재가(인사팀장)
		if(confirm("재가합니까?")){
			$("#opt").val("VV");
			$("#fm1").attr({action:"flex_x.asp", method:"post"}).submit();
		}
			...
			<tbody>
				...
				<tr height="24">
					...
					<td>
						...
						<button type="button" id=btnSearch class="">찾기</button>
					</td>
				</tr>
				<tr>
					<th>적용일자</th>
...
'		Response.Write "val : "& Date() - CDate(arrChk(i)) &"<br>"
		If Date() - CDate(arrChk(i)) < 0 Then flag = flag + 1	'미래면 +1
...
%>
				...
				<tr>
					<th>적용근무시간</th>
					<td>
						<select name="stime" id="stime" class="sel3" style="width:100px;">
							<%=tmList(stime)%>
						</select>
						~
						<select name="etime" id="etime" class="sel3" style="width:100px;">
							<%=tmList(etime)%>
						</select>
					</td>
				</tr>
...
<%
	If seq = "" Then
%>
						<input type="text" name="sign1" id="sign1" style="width:150px;" class="ct" readonly value="인사팀장" />
<%
	Else
...
<%
		Else								'서명이 안된 경우
%>
						<input type="text" name="sign1" id="sign1" value="<%=sign1%>" class="ct" readonly style="width:150px;" />
<%
		End If
	End If
%>
		...
		<div class="ct mt10">
<%
	If seq <> "" And adm_chk <> "Y" Then		'인사팀장 재가 전
%>
			<!-- <button type="button" id=btnSanc class="btn03">재가</button> -->
			<!-- <button type="button" id=btnBundleSanc class="btn02">일괄재가</button> -->
<%
	End If
%>
			<button type="button" id=btnSave class="btn02">저장</button>
<%
	If seq <> "" Then	'수정
%>
